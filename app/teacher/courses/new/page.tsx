...
function NewCourseContent() {
  ...
  // Step 2: Curriculum Structure in memory
  ...
  const validateStep1 = () => {
    ...
    if (titleTrimmed.length < 3) {
      newErrors.title = isArabic ? 'عنوان الكورس يجب أن يتكون من 3 أحرف على الأقل' : 'Course title must be at least 3 characters';
    }
    if (descTrimmed.length < 10) {
      newErrors.description = isArabic ? 'وصف الكورس يجب أن يتكون من 10 أحرف على الأقل' : 'Course description must be at least 10 characters';
    }
  ...
  const handlePublishCourse = async () => {
    ...
    try {
      ...
      if (!courseRes.ok || !courseData.course) {
        throw new Error(courseData.message || (isArabic ? 'فشل حفظ الكورس' : 'Failed to save course'));
      }
      ...
      alert(err.message || (isArabic ? 'حدث خطأ أثناء حفظ الكورس' : 'Error saving course'));
      setIsSaving(false);
    }
  };
  ...
  return (
    <div className="w-full max-w-6xl mx-auto px-4 sm:px-6 py-8 md:py-12 flex flex-col gap-8 text-start min-h-[85dvh]">
      ...
      <div className="flex flex-col gap-6 border-b border-black/5 pb-6">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            ...
            <div>
              <div className="flex items-center gap-2 mb-0.5">
                ...
                <span className="text-[11px] font-bold text-gold">
                  {currentStep === 1 
                    ? (isArabic ? 'الخطوة 1: البيانات الأساسية' : 'Step 1: Course Info') 
                    : (isArabic ? 'الخطوة 2: بناء المنهج والمحاضرات' : 'Step 2: Curriculum Structure')
                  }
                </span>
              </div>
              <h1 className="font-display font-bold text-2xl sm:text-3xl text-forest">
                {currentStep === 1 
                  ? (isArabic ? 'إضافة كورس دراسي جديد' : 'Create New Course') 
                  : (isArabic ? `بناء محتوى: ${formData.title || 'المنهج'}` : `Curriculum: ${formData.title || 'Course'}`)
                }
              </h1>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 w-full sm:w-auto">
            {currentStep === 1 ? (
              ...
              >
                <span>{isArabic ? 'التالي: بناء المنهج والمحاضرات' : 'Next: Build Curriculum'}</span>
                {isArabic ? <ArrowLeft size={16} weight="bold" /> : <ArrowRight size={16} weight="bold" />}
              </button>
            ) : (
              <div className="flex items-center gap-3 w-full sm:w-auto">
                ...
                >
                  ...
                  <span>{isArabic ? 'السابق (تعديل البيانات)' : 'Back (Edit Info)'}</span>
                </button>
                ...
                >
                  {saveSuccess ? (
                    <>
                      ...
                      <span>{isArabic ? 'تم النشر بنجاح!' : 'Published Successfully!'}</span>
                    </>
                  ) : (
                    <>
                      <FloppyDisk size={18} weight="bold" />
                      <span>{isSaving ? (isArabic ? 'جاري نشر الكورس...' : 'Publishing...') : (isArabic ? 'حفظ ونشر الكورس بالكامل' : 'Publish Course')}</span>
                    </>
                  )}
                </button>
              </div>
        ...
        <div className="grid grid-cols-2 gap-3 max-w-xl">
          ...
          >
            ...
            <div>
              <span className="font-bold text-xs block">{isArabic ? 'البيانات الأساسية والغلاف' : 'General Information'}</span>
              <span className={`text-[10px] block ${currentStep === 1 ? 'text-gold/80' : 'text-forest/40'}`}>
                {isArabic ? 'العنوان، المادة، والسعر' : 'Title, subject & grade'}
              </span>
            </div>
          </button>
          ...
          >
            ...
            <div>
              <span className="font-bold text-xs block">{isArabic ? 'المنهج والمحاضرات' : 'Curriculum & Lectures'}</span>
          ...
          <div className="lg:col-span-7 flex flex-col gap-6">
            <div className="bg-white rounded-3xl p-6 sm:p-8 border border-black/5 shadow-xs flex flex-col gap-5">
              <div className="flex items-center gap-2 border-b border-black/5 pb-3">
                ...
                <h2 className="font-display font-bold text-lg text-forest">{isArabic ? 'البيانات الأكاديمية للكورس' : 'Academic Course Details'}</h2>
              </div>
              ...
              <Input 
                ...
                onChange={e => {
                  ...
                  if (touched.title) {
                    setErrors(prev => ({
                      ...prev,
                      title: e.target.value.trim().length < 3 ? (isArabic ? 'عنوان الكورس يجب أن يتكون من 3 أحرف على الأقل' : 'Title must be at least 3 chars') : undefined
                    }));
                  }
                }} 
                onBlur={() => setTouched(prev => ({ ...prev, title: true }))}
                placeholder={isArabic ? "مثال: مراجعة ليلة الامتحان في الجغرافيا السياسية" : "e.g. Comprehensive Secondary Final Review"}
                required 
              />
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    {teacherSubjects.length === 1 && (
                      <span className="text-[10px] font-bold text-gold bg-forest/5 px-2 py-0.5 rounded-md">
                        {isArabic ? 'تخصصك المعتمد' : 'Verified'}
                      </span>
                    )}
                  </div>
                  <select 
                ...
                <div className="flex flex-col gap-1.5 w-full">
                  <label className="text-xs font-bold text-forest">{isArabic ? 'الصف الدراسي' : 'Academic Grade'}</label>
                  <select 
                    className="w-full bg-[#F7F6F3] focus:bg-white border border-transparent focus:border-gold rounded-xl px-3.5 py-3 text-xs sm:text-sm text-forest outline-none font-medium cursor-pointer"
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  ...
                  >
                    ...
                    <div>
                      ...
                      <span className={`text-[10px] block ${!formData.isFree ? 'text-gold/80' : 'text-forest/50'}`}>
                        {isArabic ? 'يتطلب كود تفعيل للوصول' : 'Protected course content'}
                      </span>
                    </div>
                  </div>
                  ...
                  >
                    ...
                    <div>
                      ...
                      <span className={`text-[10px] block ${formData.isFree ? 'text-gold/80' : 'text-forest/50'}`}>
                        {isArabic ? 'متاح لجميع الطلاب فوراً' : 'Available for all students'}
                      </span>
                    </div>
                  </div>
                </div>
              ...
              <div className="flex flex-col gap-1.5 w-full">
                <div className="flex items-center justify-between">
                  <label className="text-xs font-bold text-forest">
                    {isArabic ? 'وصف المنهج ومحتويات الكورس' : 'Curriculum Description'} <span className="text-rose-500">*</span>
                  </label>
                ...
                <textarea 
                  ...
                  placeholder={isArabic ? "نبذة شاملة عن محتويات المنهج وما سيحصل عليه الطالب من شروحات وحل نماذج امتحانات..." : "Comprehensive overview of the course content and learning objectives..."}
                  required
                />
                {touched.description && errors.description && (
          ...
          <div className="lg:col-span-5 flex flex-col gap-6">
            ...
            <div className="bg-white rounded-3xl p-6 border border-black/5 shadow-xs flex flex-col gap-4">
              <h3 className="font-display font-bold text-base text-forest border-b border-black/5 pb-2">
                {isArabic ? 'غلاف الكورس' : 'Course Cover Art'}
              </h3>
              ...
              <div className="aspect-video w-full rounded-2xl overflow-hidden bg-forest/5 relative border border-black/5">
                {formData.coverImage ? (
                  ...
                  <div className="w-full h-full flex items-center justify-center text-forest/40 text-xs">
                    {isArabic ? 'بدون صورة غلاف' : 'No Cover Image'}
                  </div>
                )}
              </div>
              ...
              >
                ...
                {isUploadingImage ? (
                  <div className="flex flex-col items-center gap-2 py-3 text-forest">
                    ...
                    <span className="text-xs font-bold">{isArabic ? 'جاري رفع الصورة' : 'Uploading image...'}</span>
                  </div>
                ) : (
                  <>
                    ...
                    <span className="text-xs font-bold text-forest">{isArabic ? 'اضغط لاختيار صورة الغلاف من جهازك' : 'Click to select cover art'}</span>
            ...
            <div className="bg-white rounded-3xl p-6 border border-black/5 shadow-xs flex flex-col gap-4">
              <div>
                <h4 className="font-display font-bold text-sm text-forest mb-1">
                  {isArabic ? 'جاهز لبناء المحتوى والدروس؟' : 'Ready to build lessons?'}
                </h4>
                <p className="text-xs text-forest/65 leading-relaxed">
                  {isArabic 
                    ...
                    : 'In step 2, you will organize curriculum chapters, link lectures, and set up quizzes.'
                  }
                </p>
              </div>
              ...
              >
                <span>{isArabic ? 'الانتقال لبناء المنهج والمحاضرات' : 'Proceed to Syllabus Builder'}</span>
                {isArabic ? <ArrowLeft size={16} weight="bold" /> : <ArrowRight size={16} weight="bold" />}
              </button>
            </div>
      ...
      {currentStep === 2 && (
        ...
        >
          ...
          <div className="bg-white rounded-3xl p-6 sm:p-8 border border-black/5 shadow-xs flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6">
            <div>
              ...
              <p className="text-xs text-forest/60 mt-1">
                {isArabic ? 'أضف وحدات المنهج، ونظم محاضرات الفيديو والاختبارات التقييمية لكل وحدة.' : 'Create chapters, add video lessons, and interactive question banks.'}
              </p>
            </div>
            ...
            <div className="bg-white rounded-3xl p-12 border border-black/5 shadow-xs text-center flex flex-col items-center justify-center gap-4 py-16">
              ...
              <div className="max-w-sm">
                <h3 className="font-display font-bold text-lg text-forest mb-1">
                  {isArabic ? 'لا توجد وحدات في هذا المنهج بعد' : 'No chapters added yet'}
                </h3>
                <p className="text-xs text-forest/60 leading-relaxed">
                  {isArabic 
                    ...
                    : 'Start by adding your first curriculum chapter, then add lectures and quizzes.'
                  }
                </p>
              </div>
              <Button 
                  ...
                  <div className="flex flex-col gap-2.5">
                    {section.items.length === 0 ? (
                      <div className="p-6 rounded-2xl bg-[#F7F6F3] border border-dashed border-black/10 text-center text-forest/40 text-xs">
                        {isArabic ? 'لا توجد محاضرات أو اختبارات داخل هذه الوحدة حتى الآن.' : 'No lectures or quizzes added yet.'}
                      </div>
                    ) : (
                            ...
                            <div className="min-w-0">
                              ...
                              <span className="text-[10px] text-forest/50 block">
                                {item.type === 'video' 
                                  ...
                                  : (isArabic ? `اختبار إلكتروني (${item.questions?.length || 0} أسئلة)` : `Interactive Quiz (${item.questions?.length || 0} questions)`)
                                }
                              </span>
                            </div>
                          </div>
          ...
          <div className="bg-white rounded-3xl p-6 border border-black/5 shadow-xs flex flex-col sm:flex-row items-center justify-between gap-4 mt-4">
            ...
            >
              {isArabic ? <ArrowRight size={16} weight="bold" /> : <ArrowLeft size={16} weight="bold" />}
              <span>{isArabic ? 'العودة للبيانات الأساسية' : 'Return to Course Info'}</span>
            </button>
            ...
            >
              {saveSuccess ? (
                <>
                  ...
                  <span>{isArabic ? 'تم حفظ ونشر الكورس بنجاح!' : 'Course Published Successfully!'}</span>
                </>
              ) : (
                <>
                  <FloppyDisk size={20} weight="bold" />
                  <span>{isSaving ? (isArabic ? 'جاري نشر الكورس...' : 'Publishing...') : (isArabic ? 'حفظ ونشر الكورس بالكامل' : 'Publish Complete Course')}</span>
                </>
              )}
            </button>
          </div>
      ...
      <PromptModal
        ...
        description={isArabic ? "أدخل عنوان الوحدة أو الفصل الدراسي لإضافة المحاضرات والاختبارات بداخله." : "Enter the title of the chapter to organize lectures and quizzes."}
        placeholder={isArabic ? "مثال: الوحدة الثانية - القوى السياسية والدول" : "e.g. Chapter 2 - Advanced Geometry"}
      />
      ...
      <PromptModal
        ...
        description={isArabic ? "قم بتعديل اسم أو رقم هذه الوحدة." : "Update the title or number for this chapter."}
