...
function TeacherDashboardContent() {
  ...
  const stats = [
    { label: 'إجمالي الطلاب المسجلين', value: totalEnrolledStudents, icon: Users, color: 'bg-forest text-gold' },
    { label: 'الكورسات المنشورة', value: myCourses.length, icon: Book, color: 'bg-emerald-50 text-emerald-700' },
    { label: 'أكواد سنتر مفعّلة', value: usedCodesCount, icon: Ticket, color: 'bg-gold/20 text-forest' },
    { label: 'تسليمات الاختبارات', value: submissions.length, icon: ClipboardText, color: 'bg-forest/5 text-forest' },
  ];
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 border-b border-black/5 pb-8">
        <div>
          <h1 className="font-display font-bold text-2xl sm:text-3xl md:text-4xl text-forest mb-1">
            أهلاً بك، {currentUser?.name || 'أستاذ المادة'}
          </h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 flex flex-col gap-6">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="font-display font-bold text-xl text-forest">كورساتي الدراسية</h2>
              <p className="text-xs text-forest/60">المناهج والمحاضرات المنشورة على المنصة</p>
            </div>
            {myCourses.length > 0 && (
          ...
          {myCourses.length === 0 ? (
            <div className="bg-white rounded-3xl p-10 border border-black/5 shadow-xs text-center flex flex-col items-center justify-center gap-4 py-16">
              ...
              <div className="max-w-sm">
                <h3 className="font-display font-bold text-lg text-forest mb-1">لم تقم بإنشاء أي كورسات بعد</h3>
                  ...
                  >
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <span className="text-[10px] font-bold px-2 py-0.5 rounded-md bg-forest/5 text-forest inline-block mb-1">
                          {course.grade === 'sec3' ? 'الصف الثالث الثانوي' : 'المرحلة الدراسية'}
                        </span>
                        <h3 className="font-bold text-sm text-forest truncate">{course.title}</h3>
                        <span className="text-xs text-forest/50 block mt-0.5">{enrolledCount} طالب مشترك</span>
                      </div>
                    </div>
                    ...
                    <div className="pt-3 border-t border-black/5 flex items-center justify-between gap-2">
                      ...
                      <Link href={`/student/course/${course.id}`}>
                        <button className="p-2 rounded-xl bg-white hover:bg-black/5 border border-black/5 text-forest/70 hover:text-forest transition-colors cursor-pointer" title="معاينة كطالب">
        ...
        <div className="flex flex-col gap-6">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="font-display font-bold text-xl text-forest">أحدث التسليمات</h2>
              <p className="text-xs text-forest/60">نتائج وحلول الطلاب الأخيرة</p>
            </div>
          ...
          <div className="bg-white rounded-3xl p-5 border border-black/5 shadow-xs flex flex-col gap-3">
            {submissions.length === 0 ? (
              <div className="py-12 text-center text-forest/40 flex flex-col items-center justify-center gap-2">
                ...
                <p className="text-xs font-medium">لا توجد تسليمات اختبارات جديدة حتى الآن</p>
              </div>
            ) : (
              submissions.slice(0, 5).map((sub: any) => (
                ...
                >
                  <div className="min-w-0">
                    <span className="font-bold text-forest block truncate">{sub.studentName || 'طالب نُـخبة'}</span>
                    <span className="text-[10px] text-forest/50 block truncate">{sub.quizTitle || 'اختبار تقييمي'}</span>
                  </div>
